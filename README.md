...
- **고성능 메모리 관리:** `ObjectPool`과 `ThreadLocalCache`를 도입하여 객체 재사용을 극대화하고 락 경합을 줄여 전체적인 처리량을 높입니다.

## Project Structure
- `braid/` — 핵심 서버 타깃 및 소스 트리로, 서비스/네트워크/스레드/태스크/유틸 계층을 포함합니다.
  - `net/` — io_uring 기반의 IO 요청·완료 처리 로직과 관련 헤더·구현.
  - `memory/` — 객체 풀링 및 스레드 로컬 캐시를 통한 효율적인 메모리 관리 모듈.
...
## Development
- 서비스는 현재 고정 개수의 `ServiceSession` 세션을 초기화하고 IO 요청을 워커 스레드 간 라운드로빈으로 분배합니다.
